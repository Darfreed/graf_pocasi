...
class Zaciform(Form):
    ...
    pohlavi = BooleanField('Pohlavi')
    trida = IntegerField("Trida",validators=[InputRequired(message="You can't leave this empty")])

class Testform(Form):

    prvni = TextField('Jmeno', validators=[
        Predicate(safe_characters, message="Please use only letters (a-z) and numbers"),
        Length(min=1, max=30, message="Please use between 3 and 30 characters")
    ])
    druhy = TextField('Prijmeni', validators=[
        Predicate(safe_characters, message="Please use only letters (a-z) and numbers"),
        Length(min=1, max=30, message="Please use between 3 and 30 characters")
    ])